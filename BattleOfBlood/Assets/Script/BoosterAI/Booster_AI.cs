...
public class Booster_AI : MonoBehaviour
{
    ...
    private Sequence seqDetect = new Sequence();          // Enemy를 없애는 기능을 하는 sequence 생성
    ...
    void Start()
    {
        ...
        selector.AddChild(seqDetect);       // seqRevive 노드를 selector의 자식 노드로 연결
        ...
        seqDetect.AddChild(boosterobstacleDetect);  //seqRevive 노드에 클래스 변수를 자식으로 추가
        seqDetect.AddChild(boosterTeamPosDetect);
        seqDetect.AddChild(boosterEnemyPosDetect);
